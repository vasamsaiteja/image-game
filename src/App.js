...
class App extends Component {
  state = {
    ...
    isGameStarted: true,
    activeImageUrl: imagesList[Math.ceil(Math.random() * imagesList.length)],
    score: 0,
    timer: 60,
  }
  ...
  imageCorrect = async newId => {
    const {activeImageUrl} = this.state

    console.log('fromImageItem', newId)
    console.log('src', activeImageUrl.id)
    if (newId === activeImageUrl.id) {
      ...
      this.setState({isGameStarted: false, timer: 0})
    }
  }
  ...
  render() {
    ...
    const srcImage = activeImageUrl.imageUrl
      ...
      <>
        ...
        {isGameStarted ? (
          <div className="background-container">
            {/* <p>Image has to be placed</p> */}

            {/* {this.getImageUrl()} */}
            <img src={srcImage} alt="match" className="image-icon" />
            ...
            <ul className="image-container">
              {filteredList.map(imageDetails => (
                <ImageItem
                  key={imageDetails.id}
                  imageDetails={imageDetails}
                  imageCorrect={this.imageCorrect}
                />
              ))}
            </ul>
          </div>
